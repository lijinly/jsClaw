...
Sheepy 是一个智能任务编排 Agent，位于用户和 Worker Agent 之间，负责：
...
// 调用 Sheepy
const result = await runSheepy('帮我读取当前目录下的文件', {
  verbose: true,  // 打印中间过程
});
...
const result = await runSheepy('检查文件是否存在', { history });
```
...
- `src/sheepy.js` — Sheepy 核心实现
- `src/agent.js` — Worker Agent（Think-Act 模式）
- `src/demo-sheepy.js` — 演示示例
