...
参考 [Hermes Agent](https://github.com/nousresearch/hermes-agent) 的能力体系，规划以下改进方向：

### ⭐⭐⭐ 高优先级

| 任务 | 说明 | 对标 Hermes |
|------|------|-------------|
| **SQLite FTS5 记忆升级** | 引入全文搜索，自动评估值得保存的信息 | 零人工维护的记忆 |
| **Skill 自动生成** | 任务完成后自动生成 SKILL.md，越用越强 | skill_manage 工具 |

### ⭐⭐ 中优先级

| 任务 | 说明 | 对标 Hermes |
|------|------|-------------|
| **子代理并行执行** | spawn 隔离子代理，复杂任务并行处理 | 分布式任务架构 |
| **Cron 定时任务** | 内置定时调度器，支持自然语言配置 | 内置 Cron |
| **MCP 协议集成** | 原生支持 Model Context Protocol | MCP 原生集成 |

### ⭐ 低优先级

| 任务 | 说明 | 对标 Hermes |
|------|------|-------------|
| **消息网关** | 接入企业微信/飞书/钉钉等平台 | 15+ 平台接入 |
| **安全沙盒** | 命令审批 + Docker 容器隔离 | 企业级安全 |
| **GEPA 进化引擎** | Prompt 自动优化，类反向传播调参 | GEPA 自我进化 |
